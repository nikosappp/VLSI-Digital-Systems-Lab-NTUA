...
architecture behavioral of control_unit is
    ...
    signal dim_reg         : integer range 0 to 1024 := 0;
    signal total_pixels    : integer range 0 to 1048576 := 0;

    signal delay_cnt : integer range 0 to 2050 := 0; 
    signal col_cnt   : integer range 0 to 1023 := 0;
    signal row_cnt   : integer range 0 to 1023 := 0;

    -- Pipeline Drain Tracking
    signal pixels_in : integer range 0 to 1048576 := 0;
    signal active_en : std_logic;

begin
    -- active_en is high when reading valid data or processing data
    active_en <= '1' when (valid_in = '1') or (pixels_in = total_pixels and state /= DONE and state /= IDLE) else '0';
        ...
        if rising_edge(clk) then
            ...
            if (rst_n = '0') or (image_dim_vld = '1') then
                ...
                if image_dim_vld = '1' then
                    dim_reg <= to_integer(unsigned(image_dim));
                    total_pixels <= to_integer(unsigned(image_dim)) * to_integer(unsigned(image_dim));
                end if;
            ...
            -- Normal Operation
                ...
                if valid_in = '1' and pixels_in < total_pixels then
                ...
                if active_en = '1' then
                    case state is
                        ...
                        when RUNNING =>
                            ...
                            if col_cnt = (dim_reg - 1) then
                                -- reset column counter
                                col_cnt <= 0;
                                -- image processing finished
                                if row_cnt = (dim_reg - 1) then
                                    state <= DONE;
                                    image_finished <= '1';
                                else
                                    row_cnt <= row_cnt + 1;
    ...
    bottom_edge <= '1' when (row_cnt = dim_reg - 1) else '0';
    left_edge   <= '1' when (col_cnt = 0)   else '0';
    right_edge  <= '1' when (col_cnt = dim_reg - 1) else '0';

    -- Neigborhood Flags
    process(row_cnt, col_cnt)
        variable row_lsb, col_lsb : std_logic;
    begin
        row_lsb := std_logic(to_unsigned(row_cnt, 10)(0)); 
        col_lsb := std_logic(to_unsigned(col_cnt, 10)(0));
