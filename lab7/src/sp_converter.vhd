...
architecture behavioral of sp_converter is
    ...
    -- Gated signals to protect FIFOs during pipeline stalls
    signal wr_en2_gated, rd_en2_gated : std_logic;
    signal wr_en3_gated, rd_en3_gated : std_logic;
    -- Fill Counters (track when N pixels have been buffered)
    signal cnt1, cnt2, cnt3 : integer range 0 to 1023 := 0;
    -- Inverted reset for the FIFO IP
    signal rst : std_logic;

    -- dimension signal
    signal dim_int : integer range 0 to 1024;
...
begin
    ...
    dim_int <= to_integer(unsigned(image_dim));

    rst <= (not rst_n) or image_dim_vld;
    ...
    begin
        if rising_edge(clk) then
            ...
            if (rst_n = '0') or (image_dim_vld = '1') then
    ...
    rd_en1 <= '1' when (enable = '1' and cnt1 = (dim_int - 1)) else '0';
    rd_en2 <= '1' when (wr_en2 = '1' and cnt2 = (dim_int - 1)) else '0';
    rd_en3 <= '1' when (wr_en3 = '1' and cnt3 = (dim_int - 1)) else '0';
    ...
    -- Use gated wr/rd enables for fifo_2 and fifo_3
    wr_en2_gated <= wr_en2 and enable;
    rd_en2_gated <= rd_en2 and enable;
    wr_en3_gated <= wr_en3 and enable;
    rd_en3_gated <= rd_en3 and enable;
